...
HS93: Challenging nonlinear (6 vars, 2 ineq)
  min  0.0204*x1*x4*(x1+x2+x3) + 0.0187*x2*x3*(x1+1.57*x2+x3)
       + 0.0607*x1*x4*x5^2*(x1+x2+x3) + 0.0437*x2*x3*x6^2*(x1+1.57*x2+x3)
  s.t. 0.001*x1*x2*x3*x4*x5*x6 - 2.07 >= 0
       0.00062*x1*x4*x5^2*(x1+x2+x3) + 0.00058*x2*x3*x6^2*(x1+1.57*x2+x3) - 1 >= 0
       0 <= xi
...
class HS93(am.Component):
    ...
    def compute(self):
        ...
        self.constraints["c2"] = (
            0.00062 * x1 * x4 * x5**2 * s1 + 0.00058 * x2 * x3 * x6**2 * s2 - 1
        )
